...
%
function [Opt_q,X,P] = USQUE(a,f,Q,R,n,lambda,T,X,q,omega,obs,P)
...
for i=1:sigmadots
    sq(:,i) =qAntiMatrix(q)*delta_q(:,i);
end
est_omega=repmat(omega,1,sigmadots)-aleph(4:6,:);
%% prediction
...
for i=1:sigmadots
    pred_delta_q(:,i)=qAntiMatrix([pred_q(1,1);-pred_q(2:4,1)])*pred_q(:,i);
end
...
X=pred_mean+KGain*(obs-obs_mean);
...
Opt_q=qAntiMatrix(pred_q(:,1))*opt_delta_q;
