...
%% 误差产生
if( t==0 )
    %       Gyro_r=zeros(3,1);    %陀螺一阶马尔可夫过程
%     Gyro_b=0.1*deg_rad/3600.0*randn(3,1); %随机常数0.01(deg/h)
    %       Gyro_r=3.1623e-10*randn(3,1); %陀螺一阶马尔可夫过程0.01(deg/h)
    %        Gyro_wg=0.31623*(10^-7)*randn(3,1);%陀螺白噪声0.138(deg/s)
    %     Gyro_r=0.00551925469358167e-8*randn(3,1); %陀螺一阶马尔可夫过程0.01(deg/h)
    %     Gyro_wg= 0.00551925469358167e-6*randn(3,1);%陀螺白噪声0.138(deg/s)
    Gyro_r=Gyro_r+noise_wARW*T*randn(3,1);
    Gyro_wg= noise_gyro*randn(3,1);
else
    %       Gyro_wr=0.01*sqrt(2*T/Tg)*deg_rad/3600.0*randn(3,1);%陀螺一阶马尔可夫过程白噪声0.01(deg/h)
    %       Gyro_r=exp(-1.0*T/Tg)*Gyro_r+Gyro_wr;%陀螺一阶马尔可夫过程0.01(deg/h)
    %       Gyro_wg=500*deg_rad/3600.0*randn(3,1);%陀螺白噪声0.277(deg/s)
    %       Gyro_wr=3.1623e-4*sqrt(2*T/Tg)*(10^-6)*randn(3,1);%陀螺一阶马尔可夫过程白噪声3.1626*10^-4rad/(s^(3/2) )
    %       Gyro_r=exp(-1.0*T/Tg)*Gyro_r+Gyro_wr;%陀螺一阶马尔可夫过程0.01(deg/h)
    %         Gyro_r=Gyro_r+3.1623e-9*randn(3,1);
    %
    %       Gyro_wg=0.31623*(10^-6)*randn(3,1);%陀螺白噪声0.31623(rad/s)
    %         Gyro_r=Gyro_r+3.1623e-10*randn(3,1);
    %
    %       Gyro_wg=0.31623e-6*randn(3,1);%陀螺白噪声0.31623(rad/s)0.31623e-6
    Gyro_r=Gyro_r+noise_wARW*T*randn(3,1);
